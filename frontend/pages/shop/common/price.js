...
const Price = () => {
  ...
  const [values, setValues] = useState([500, 10000]);
  ...
  const priceHandle = (value) => {
    if (value) {
      ...
      setValues(value);
      router.push(`${url}?${context.state}&brand=${context.selectedBrands}&color=${context.selectedColor}&size=${context.selectedSize}&minPrice=${context.selectedPrice?.min}&maxPrice=${context.selectedPrice?.max}`, undefined, { shallow: true });
    }
  };
